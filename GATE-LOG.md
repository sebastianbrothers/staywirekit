...
| Signature reconciliation | **ADJUDICATED: wire indigo is canon** | — | Kit is the brand source of truth; the product admin re-themes onto @staywirekit/ui tokens in a future product lane (replacing the interim paper/ink/copper theme). |
| Voice acceptance | **ADJUDICATED: product-noun led** — hero claims rewritten | this commit | hero6 "The booking engine for independent stays" · hero1 "Availability, quotes, bookings — one API" · hero2 "A booking engine guests never notice" · hero3 "A booking flow guests finish" · hero4 "The booking engine for agents" · close "The booking wire. / Live today." Pricing names (Test/Live/Portfolio) accepted as shipped |
| Rhythm eye-tune | RESOLVED via morning revision 1 | d6c0bfa | Root cause was phantom utilities, not token values; 96/128 rhythm confirmed by operator screenshots post-fix |
| Phase 0 intake | **RATIFIED** (operator, morning review) | INTAKE.md | Fonts, light derivation, synthesized dark, spacing knobs all stand. Every gate in this log is now CLOSED — v0.1.0+ is the accepted baseline. |
